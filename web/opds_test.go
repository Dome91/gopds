...
func TestDownload(t *testing.T) {
	fetchCatalogEntryByID := func(id string) (domain.CatalogEntry, error) {
		assert.Equal(t, "id1", id)
		return domain.CatalogEntry{Name: "Penguin Island.epub", Path: "../test/ebooks/category1/Penguin Island.epub"}, nil
	}
	...
	assert.Nil(t, err)
	assert.Equal(t, http.StatusOK, response.StatusCode)
	assert.Equal(t, "filename=Penguin Island.epub", response.Header.Get(fiber.HeaderContentDisposition))
	assert.Equal(t, "678839", response.Header.Get(fiber.HeaderContentLength))
}
